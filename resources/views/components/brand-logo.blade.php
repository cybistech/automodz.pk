...
<div {{ $attributes->merge(['class' => 'inline-flex flex-col overflow-visible']) }}>
    ...
    @else
        <svg
            class="{{ $height }} w-auto max-w-none shrink-0 overflow-visible"
            viewBox="0 0 296 64"
            fill="none"
        ...
        >
            <defs>
                <linearGradient id="{{ $uid }}-fire" x1="70" y1="32" x2="300" y2="32" gradientUnits="userSpaceOnUse">
                ...
                <filter id="{{ $uid }}-soft" x="-10%" y="-10%" width="120%" height="120%">
                    <feDropShadow dx="0" dy="1" stdDeviation="1.5" flood-color="#F97316" flood-opacity="0.28"/>
                </filter>
            </defs>
            ...
            <text x="72" y="41" font-family="Rajdhani, Arial Narrow, Arial, sans-serif" font-weight="700" font-size="38" fill="#F8FAFC" letter-spacing="-1">Auto</text>
            <text x="141" y="41" font-family="Rajdhani, Arial Narrow, Arial, sans-serif" font-weight="700" font-size="38" fill="url(#{{ $uid }}-fire)" letter-spacing="-1" filter="url(#{{ $uid }}-soft)">Modz</text>
    ...
    @if($showTagline && ! $iconOnly)
        <p class="{{ $taglineSize }} mt-1 font-medium leading-snug tracking-wide text-slate-400">{{ config('site.tagline') }}</p>
    @endif
</div>
